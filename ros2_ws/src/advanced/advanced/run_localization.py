...
class Localization:
    ...
    def load_map(self):
        ...
        self.map=np.reshape(np_map_data,(600, 600))
    ...
    #     # map_data 전체 읽기 → reshape 에 맞게
    #     np_map_data = np.array([int(x) for x in line_data])

    #     # 💡 reshape 하기 전에 길이 체크!
    #     expected_size = int(self.map_size[0] * self.map_size[1])
    #     if len(np_map_data) != expected_size:
    #         raise ValueError(f"[❌] map 데이터 길이 불일치: {len(np_map_data)}개 (예상 {expected_size}개)")

    #     self.map = np.reshape(np_map_data, (int(self.map_size[0]), int(self.map_size[1])))
    #     self.map = ((100 - self.map) * 255.0 / 100.0).astype(np.uint8)
    ...
    #     print(f"[디버깅] map_size: {self.map_size}, 전체 요소 수: {int(self.map_size[0] * self.map_size[1])}")
    #     print(f"[디버깅] line_data 길이: {len(line_data)}")
